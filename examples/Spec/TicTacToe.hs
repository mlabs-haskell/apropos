...
data Board = Board Integer Integer [Maybe Player] deriving stock (Show,Eq)

initialBoard :: Board
initialBoard = Board 3 3 $ replicate 9 Nothing
...
--  satisfiesProperty (MoveProposal f t _ _) PlacementLegal = placementLegal f t
  ...
  logic = Yes --All [ Var GameInInitialState :->: (All $ Var <$> [FromIs3by3,FromBoardReachable,PlacementLegal])
  ...
  expect = Yes --All $ Var <$> [ IsPlayersTurn
               --          , FromIs3by3
               --          , ToIs3by3
               --          , SingleTileDiff
               --          , FromBoardReachable
  ...
  genBaseModel = do
    player' <- element [X,O]
    currentState <- list (linear 9 9) (element [Nothing])
    nextState <- list (linear 6 24) (element [Nothing,Just X,Just O])
    win <- Gen.bool
    return $ MoveProposal (Board 3 3 currentState) (Board 3 3 nextState) player' win
 
  transformation (On GameInInitialState) m = return $ m { from = initialBoard }
  transformation (Off GameInInitialState) m = do
    board <- list (linear 9 9) (element [Nothing])
    pure $ m { from = Board 3 3 board }
