...
instance HasPermutationGenerator PlayerProperty Int where
  generators =
    ...
    , PermutationEdge
      ...
      , permuteGen = do
          p <- liftGenPA $ Gen.filter (\i -> not (i `elem` [0,1]))
...
playerPermutationGenSelfTest = testGroup "playerPermutationGenSelfTest" $
  fromGroup <$> permutationGeneratorSelfTest True
                (\(_ :: PermutationEdge PlayerProperty Int) -> True) baseGen
