...
runTest :: forall (a :: Type) (d :: Type). (Show a, Description d a) => (a -> PropertyT IO ()) -> d -> Property
runTest cond d = property $ forAll (genDescribed d) >>= cond

decorateTests :: forall (s :: Type) (d :: Type). (IsString s, Show d) => Map d Property -> [(s, Property)]
...
selfTestForDescription :: forall (d :: Type) (a :: Type). (Eq d, Show d, Show a, Description d a) => d -> Property
...
-}
selfTest :: forall (d :: Type) (a :: Type) (s :: Type). (Ord d, Show d, Show a, Description d a, IsString s) => Proxy d -> [(s, Property)]
