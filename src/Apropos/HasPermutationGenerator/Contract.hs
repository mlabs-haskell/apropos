...
runContract :: (Enumerable p, LogicalModel p) => Contract p () -> Set p -> Either String (Set p)
runContract c s =
...
interprets :: Enumerable p => [Instruction p] -> Set p -> Set (Set p)
...
interpret :: forall p. (Enumerable p) => Instruction p -> Set p -> Set (Set p)
