...
translateInstruction (Adds ps) = EdgeFormula
  (All [ Var (S 0 v) :<->: Var (S 1 v) | v <- enumerated , v `notElem` ps ] :&&: All [(Var (S 0 p) :||: Not (Var (S 0 p))) :&&: Var (S 1 p) | p <- ps]) 1 No 0 1
translateInstruction (Removes ps) = EdgeFormula
  (All [ Var (S 0 v) :<->: Var (S 1 v) | v <- enumerated , v `notElem` ps ] :&&: None [(Var (S 0 p) :||: Not (Var (S 0 p))) :&&: Var (S 1 p) | p <- ps]) 1 No 0 1
translateInstruction (Holds f) = EdgeFormula (S 0 <$> f) 0 No 0 0
translateInstruction (Branch bs) = let
  pbs = translateInstructions <$> bs
  ...
  newAnti = Some $ do
    ...
    pure $ All [con0a,con0b,cona1,conb2,forma,formb,
...
solveContract c =
  case solveAll (antiValidity c) of
    [] ->
      ...
      | s <- solveAll (form c) ]
    s -> let
        trues = Map.keys $ Map.filter id $ head s
        falses = Map.keys $ Map.filter not $ head s
      in error $
           "\n ambiguous branches"
        ++ "\n formula " ++ show (antiValidity c)
        ++ "\n in: true " ++ show [a | S 0 a <- trues ] ++ " false " ++ show [ a | S 0 a <- falses ]
        ++ "\n out1: true " ++ show [a | S 1 a <- trues ] ++ " false " ++ show [ a | S 1 a <- falses ]
